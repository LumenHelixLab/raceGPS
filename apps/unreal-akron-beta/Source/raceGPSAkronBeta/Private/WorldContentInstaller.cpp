...
bool UWorldContentInstaller::VerifyInstallation(FString& OutSummary)
{
	...
	for (const FPreflightCheck& Check : Checks)
	{
		if (Check.Category.Contains(TEXT("World")) || Check.Category.Contains(TEXT("City")) || Check.Description.Contains(TEXT("World")) || Check.Description.Contains(TEXT("City")))
		{
			Lines.Add(FString::Printf(TEXT("World/city check: %s | %s"), *Check.Description, *Check.Detail));
			if (!Check.RecommendedAction.IsEmpty())
			{
				Lines.Add(FString::Printf(TEXT("Recommended: %s"), *Check.RecommendedAction));
			}
		}
	}
