...
export interface VehicleState {
  ...
  speed: number;   // m/s
  x: number;       // meters east from origin
  y: number;       // meters up
  z: number;       // meters south from origin
}
...
export class RapierVehicle {
  ...
  async init(originLat: number, originLon: number): Promise<void> {
    const mod = await import('@dimforge/rapier3d-compat');
    const R = mod.default ?? mod;
    await R.init();
    this.R = R as typeof RAPIER;
    ...
    this.world.createCollider(
      this.R.ColliderDesc.cuboid(5000, 0.1, 5000),
      groundBody,
    );
    ...
    this.world.createCollider(
      this.R.ColliderDesc.cuboid(0.9, 0.5, 2.1)
        .setMassProperties(
          800,
          { x: 0, y: -0.2, z: 0 },
          { x: 100, y: 100, z: 100 },
    ...
    this.vehicle.addWheel(
      { x: -0.8, y: connectionHeight, z: 1.4 },
      { x: 0, y: -1, z: 0 },
      { x: 1, y: 0, z: 0 },
      suspensionRest,
      wheelRadius,
  ...
  update(inputs: VehicleInputs, dt: number): VehicleState | undefined {
    if (!this.ready || !this.world || !this.vehicle || !this.chassis || !this.R) return;
  ...
  reset(lat: number, lon: number, headingDeg: number): void {
    if (!this.ready || !this.chassis || !this.vehicle || !this.R) return;
    this.originLat = lat;
    this.originLon = lon;
